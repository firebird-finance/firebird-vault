...
abstract contract StrategyBase is IStrategy, ReentrancyGuard {
    ...
    function setFirebirdRouter(IFirebirdRouter _firebirdRouter) external onlyGovernance {
        if (address(firebirdRouter) != address(0)) {
            ...
            if (targetCompoundToken != address(0) && targetCompoundToken != farmingToken) IERC20(targetCompoundToken).safeApprove(address(firebirdRouter), 0);
            if (targetProfitToken != address(0) && targetProfitToken != targetCompoundToken && targetProfitToken != farmingToken) {
                IERC20(targetProfitToken).safeApprove(address(firebirdRouter), 0);
            }
        }
        ...
        if (targetCompoundToken != address(0) && targetCompoundToken != farmingToken) IERC20(targetCompoundToken).safeApprove(address(firebirdRouter), type(uint256).max);
        if (targetProfitToken != address(0) && targetProfitToken != targetCompoundToken && targetProfitToken != farmingToken) {
            IERC20(targetProfitToken).safeApprove(address(firebirdRouter), type(uint256).max);
        }
    }
    ...
    function _swapTokens(
        ...
        if (path.length > 0) {
            // use firebird
            uint256[] memory amounts = IValueLiquidFormula(firebirdRouter.formula()).getFactoryAmountsOut(firebirdRouter.factory(), _input, _output, _amount, path);
            uint256 minAmountOut = amounts[amounts.length.sub(1)].mul(slippageFactor).div(1000);

            firebirdRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens(_input, _output, _amount, minAmountOut, path, address(this), block.timestamp);
        } else {
            // use Uniswap
...
contract StrategySushiLp is StrategyBase {
    ...
    function initialize(
        ...
        if (token0 != farmingToken && token0 != targetCompoundToken) {
            ...
            IERC20(token0).safeApprove(address(firebirdRouter), type(uint256).max);
        }
    ...
    function setTokenLp(address _token0, address _token1) external onlyStrategist {
        if (token0 != address(0)) {
            if (token0 != farmingToken && token0 != targetCompoundToken && token0 != targetProfitToken) {
                ...
                IERC20(token0).safeApprove(address(firebirdRouter), 0);
            }
        }
        if (token1 != address(0)) {
            if (token1 != farmingToken && token1 != targetCompoundToken && token1 != token0 && token1 != targetProfitToken) {
                ...
                IERC20(token1).safeApprove(address(firebirdRouter), 0);
            }
        }
        token0 = _token0;
        token1 = _token1;
